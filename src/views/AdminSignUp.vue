...
async function handleAdminSignup() {
  ...
  try {
    ...
    success.value = 'Admin signup successful! Redirecting to login'
    setTimeout(() => {
      router.push('/login')
    }, 1500)
  } catch (e) {
...
<template>
  <main class="login-wrap" aria-label="Admin Signup">
    <form class="login-card" @submit.prevent="handleAdminSignup" novalidate>
      <h1 class="app-title">Youthealth</h1>
      <h2 class="login-title">Admin Signup</h2>
      <p class="login-muted">Create an admin account.</p>
